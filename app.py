...
The app intentionally presents the model as a small experimental LM rather
than a production chatbot. Generation is CPU-friendly but can be slow.
"""
import os
...
with gr.Blocks(title="Small Multilingual Transformer") as demo:
    ...
    generate_button.click(
        predict,
        inputs=[prompt, temperature, top_k, max_new_tokens, seed],
        outputs=output,
    )
    prompt.submit(
        predict,
